...
describe('<AccountTrace />', () => {
    ...
    beforeAll(() => {
        ...
        mock.onGet(`${process.env.REACT_APP_API_URL}/transaction/ABC/account/1234`).reply(200, mockData);
        mock.onGet(`${process.env.REACT_APP_API_URL}/transaction/EFG/account/1234`).reply(404,
            { 'message': 'Axios fail' })
    })

    it('good backend response', async () => {
        act(async () => {
            const form = render(<AccountTrace date="mydate" submit="mybutton" direction="forward" />);
            ...
            expect(form.findByAltText("canvas")).toBeDefined()
        })
    })
